...
*
* Description: This is the source code for the eddystone utility functions.
*
********************************************************************************
...
*  The application calls the function to populate advertisement data for Apple
*  iBeacon advertisement format. It returns the adv data buffer and length.
*
* Parameters:
*  ibeacon_uuid         : Customer beacon UUID, see specification
*                         for details for input parameters.
*  ibeacon_major_number : Beacon major number
*  ibeacon_minor_number : Beacon minor number
*  tx_power_lcl         : Measured power
*  adv_data             : Buffer of advertisement data
...
*
*******************************************************************************/
void ibeacon_set_adv_data (uint8_t ibeacon_uuid[LEN_UUID_128],uint16_t
        ibeacon_major_number,uint16_t ibeacon_minor_number,uint8_t tx_power_lcl,
...

{
    ...
    ibeacon_adv_elem[IBEACON_ADV_INDEX1].advert_type  =
            BTM_BLE_ADVERT_TYPE_MANUFACTURER;

    /* Setting Company Identifier */
    ibeacon_data[IBEACON_DATA_INDEX0] = ibeacon_company_id
            [IBEACON_DATA_COMPANY_ID_INDEX0];
    ibeacon_data[IBEACON_DATA_INDEX1] = ibeacon_company_id
            [IBEACON_DATA_COMPANY_ID_INDEX1];
    ...
    ibeacon_data[IBEACON_DATA_INDEX20] = ibeacon_major_number & BYTE_MASK;
    /* shifting by 8 to move major value 1 */
    ibeacon_data[IBEACON_DATA_INDEX21] = (ibeacon_major_number >> SHIFT_VALUE)
                                          & BYTE_MASK;

    /* Setting the Minor field */
    ibeacon_data[IBEACON_DATA_INDEX22] = ibeacon_minor_number & BYTE_MASK;
    /* shifting by 8 to move minor value 2 */
    ibeacon_data[IBEACON_DATA_INDEX23] = (ibeacon_minor_number >> SHIFT_VALUE)
                                         & BYTE_MASK;
...
*  adv data: advertisement data
*  adv len : advertisement data length
*
* Return:
*  None
*
                                ...
                                uint8_t *adv_len)
{
    ...
    memcpy(&(eddystone_adv_elem[EDDYSTONE_ADV_INDEX2].data
            [EDDYSTONE_ADV_DATA_INDEX3]), &url_data,
            len+EDDYSTONE_URL_COM_LENGTH);

    /* Copy the adv data to output buffer */
    beacon_set_adv_data(eddystone_adv_elem, EDDYSTONE_NUM_ELEM_URL, adv_data,
            adv_len);
}
                                                       ...
                                                       uint8_t frame_len)
{
    ...
    eddystone_adv_elem[EDDYSTONE_ADV_INDEX0].advert_type =
            BTM_BLE_ADVERT_TYPE_FLAG;
    eddystone_adv_elem[EDDYSTONE_ADV_INDEX0].data[EDDYSTONE_ADV_DATA_INDEX0] =
            flag;
    ...
    memcpy(eddystone_adv_elem[EDDYSTONE_ADV_INDEX1].data, eddystone_uuid,
            UUID_LENGTH);

    /* Third adv element (partial), rest is frame specific
     frame_len + advert_type (1) + uuid (2)*/
    eddystone_adv_elem[EDDYSTONE_ADV_INDEX2].len = frame_len +
            EDDYSTONE_SERVICE_DATA_LENGTH;
    eddystone_adv_elem[EDDYSTONE_ADV_INDEX2].advert_type =
...
void beacon_set_adv_data(beacon_ble_advert_elem_t *beacon_adv_elem,
        uint8_t num_elem,uint8_t adv_data[BEACON_ADV_DATA_MAX], uint8_t *adv_len)
{
    uint8_t structure_index = RESET_VALUE, buffer_index = RESET_VALUE;

    for (structure_index = RESET_VALUE; structure_index < num_elem;
         structure_index++)
    {
        ...
        /* len - 1 for advert_type */
        memcpy(&adv_data[buffer_index], beacon_adv_elem[structure_index].data,
            (beacon_adv_elem[structure_index].len - INDEX_OFFSET));
        buffer_index += beacon_adv_elem[structure_index].len - INDEX_OFFSET;
    }
